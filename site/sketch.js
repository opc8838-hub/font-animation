...
//  exportButton = createButton('保存循环'); exportButton.position(140,10); exportButton.mousePressed(saveLoop);
  prideButton = createButton('彩虹!'); prideButton.position(140,35); prideButton.mousePressed(pride);

  presetSimple = createButton('简洁'); presetSimple.position(140,height-30); presetSimple.mousePressed(simpleSet);
  presetJellyfish = createButton('水母'); presetJellyfish.position(200,height-30); presetJellyfish.mousePressed(jellyfishSet);
  presetCrown = createButton('皇冠'); presetCrown.position(270,height-30); presetCrown.mousePressed(crownSet);
  presetComplex = createButton('复杂'); presetComplex.position(330,height-30); presetComplex.mousePressed(complexSet);
  presetWeave = createButton('编织'); presetWeave.position(405,height-30); presetWeave.mousePressed(weaveSet);
  presetZebra = createButton('斑马'); presetZebra.position(465,height-30); presetZebra.mousePressed(zebraSet);
  presetHoops = createButton('圆环'); presetHoops.position(515,height-30); presetHoops.mousePressed(hoopsSet);
...
//    text("内层",145,300);
    ...
    text("圆柱半径 " + radius,15,16);
    text("圆柱数量 " + stackNum,15,46);
    text("圆柱旋转 " + rRotate,15,76);
    text("圆柱偏移 " + rOffset,15,106);
  //  line break
    text("波浪数 " + rWaveCount,15,146);
    text("波浪速度 " + rWaveSpeed,15,176);
    text("纬度  " + rWave,15,206);
    text("经度 " + rLong,15,236);
    text("涟漪 " + rZaxis,15,266);
    text("横向缩放 " + strecherXsize,15,296);
    text("纵向缩放 " + strecherYsize,15,326);
  // line break  
    text("横向缩放 " + typeX,15,366);
    text("纵向缩放 " + typeY,15,396);
    text("字重 " + typeStroke,15,426);

    text("用于平滑纬度或涟漪调整后的形态",15,470);  
    text("微调X旋转 " + xRotTweak,15,516);
    text("微调Y旋转 " + yRotTweak,15,546);
    text("微调Z旋转 " + zRotTweak,15,576);

    text("相机缩放",15,height-22);
		text("预设", 145,height-40);
