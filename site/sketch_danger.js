...
function setup() {
  ...
  saveButton = createButton('保存PNG'); saveButton.position(width/2 + 170,height - 70); saveButton.style('width','80px'); saveButton.mousePressed(savePNG);
  ...
  allYoursButton = createButton('属于你'); allYoursButton.position(width/2 - 250,height - 30); allYoursButton.mousePressed(allYoursSet);
  justOkButton = createButton('还行'); justOkButton.position(width/2 - 175,height - 30); justOkButton.mousePressed(justOkSet);
  notSoGoodButton = createButton('不太好'); notSoGoodButton.position(width/2 - 105,height - 30); notSoGoodButton.mousePressed(notSoGood);
  cheerButton = createButton('欢呼'); cheerButton.position(width/2,height - 30); cheerButton.mousePressed(beAggressive);
  dateButton = createButton('日期'); dateButton.position(width/2 + 60,height - 30); dateButton.mousePressed(todayDate);
  hopeButton = createButton('希望'); hopeButton.position(width/2 + 110,height - 30); hopeButton.mousePressed(hopesDangers);
  circleButton = createButton('圆形'); circleButton.position(width/2 + 175,height - 30); circleButton.mousePressed(circlePreset);
...
function draw() {
  ...
  textSize(10);
  textFont(font0);
  text("速度",width/2-160,22);
  text("尺寸",width/2-160,53);
  text("复杂度",width/2-160,73);
  text("字号",width/2 + 15,height-67);
  text("行距",width/2 + 15,height-47);
  textAlign(LEFT);
  text("反色",width/2 - 128,height-46);
  text("列",width/2+160,53);
  text("行",width/2+160,73);
