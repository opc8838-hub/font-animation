...
function setup() {
  ...
  marqueeSet = createButton('跑马灯'); marqueeSet.position(25,height-60); marqueeSet.mousePressed(marquee);
  subwaySet = createButton('地铁'); subwaySet.position(95,height-60); subwaySet.mousePressed(subway);
  wowSet = createButton('哇'); wowSet.position(160,height-60); wowSet.mousePressed(wow);
  stackedSet = createButton('堆叠'); stackedSet.position(210,height-60); stackedSet.mousePressed(stacks);
  oldSeaSet = createButton('旧海'); oldSeaSet.position(270,height-60); oldSeaSet.mousePressed(oldSea);
  colorSeaSet = createButton('色彩海'); colorSeaSet.position(335,height-60); colorSeaSet.mousePressed(colorSea);
  simpleSet = createButton('简单波浪'); simpleSet.position(25,height-35); simpleSet.mousePressed(simpleWave);
  simpleSet2 = createButton('简单波浪2'); simpleSet2.position(115,height-35); simpleSet2.mousePressed(simpleWave2);
  notSoWeirdSet = createButton('不太怪'); notSoWeirdSet.position(215,height-35); notSoWeirdSet.mousePressed(notSoWeird);
  racerSet = createButton('赛车'); racerSet.position(310,height-35); racerSet.mousePressed(racer);
...
function draw() {
  ...
  textSize(9);
    ...
    if(gifRecord == false){
      ...
      text("字距 " + tracking, 25, 110);

      text("数量 " + ribbonCount, 25, 170);
      text("横向间距 " + ribbonSpaceX, 25, 200);
      text("纵向间距 " + ribbonSpaceY, 25, 230);
      text("尺寸 " + ribbonSize, 25, 260);
      text("偏移 " + ribbonOffset, 25, 290);

      text("波浪尺寸 " + yWave, 25, 350);
      text("波浪速度 " + speed, 25, 380);
      text("波长 " + offset, 25, 410);
      text("倾斜 " + slope, 25, 440);

      text("预设", 25,height-70);        
        
  push();
  rotate(-PI/2);
  text("分段开关与颜色",-215,144);
  text("背景色", -290,144);
  pop();
    }
