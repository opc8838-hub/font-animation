...
function preload() {
  font = loadFont('assets/NotoSansSC-Regular.ttf');
}
...
function setup() {
  ...
  saveLoopSet = createButton('保存循环'); saveLoopSet.position(140, 10); saveLoopSet.mousePressed(saveLoop);
  prideButton = createButton('彩虹!'); prideButton.position(140,35); prideButton.mousePressed(pride);

  oneBannerSet = createButton('旗帜'); oneBannerSet.position(150,height-60); oneBannerSet.mousePressed(oneBanner);
  twistSet = createButton('扭转'); twistSet.position(220,height-60); twistSet.mousePressed(twist);
  foldsSet = createButton('折叠'); foldsSet.position(280,height-60); foldsSet.mousePressed(folds);
  flatSeaSet = createButton('平静海面'); flatSeaSet.position(330,height-60); flatSeaSet.mousePressed(flatSea);
  barberSet = createButton('理发店旋纹'); barberSet.position(395,height-60); barberSet.mousePressed(barber);
  siloSet = createButton('筒仓'); siloSet.position(450,height-60); siloSet.mousePressed(silos);
  mysterySet = createButton('神秘'); mysterySet.position(500,height-60); mysterySet.mousePressed(mystery);

  colaWaveSet = createButton('可乐波浪'); colaWaveSet.position(150,height-40); colaWaveSet.mousePressed(colaWave);
  origamiSet = createButton('折纸'); origamiSet.position(230,height-40); origamiSet.mousePressed(origami);
  craneSet = createButton('折纸二'); craneSet.position(293,height-40); craneSet.mousePressed(crane);
  blackwhiteSet = createButton('黑白'); blackwhiteSet.position(365,height-40); blackwhiteSet.mousePressed(blackwhite);
  newsprintSet = createButton('报纸'); newsprintSet.position(415,height-40); newsprintSet.mousePressed(newsprint);
  edgeSet = createButton('边缘'); edgeSet.position(490,height-40); edgeSet.mousePressed(edge);
...
//  xWave = xWaveSlider.value();
  ...
  textAlign(LEFT);
    if(gifRecord == false){
      text("文字横向缩放 " + typeX, 15, 30);
      text("文字纵向缩放 " + typeY, 15, 60);
      text("文字粗细 " + typeStroke, 15, 90);
      text("行数  " + rows, 15, 120);

      text("文字间距 " + paddingSlider.value(), 15, 150);

      text("横向尺寸  " + xWaveSlider.value(), 15, 210);
      text("纵向尺寸  " + yWaveSlider.value(), 15, 240);
      text("纵深尺寸  " + zWave, 15, 270);
      text("波浪偏移  " + offset, 15, 300);
      text("波浪速度  " + -speed, 15, 330);
      text("行偏移  " + rowOffset, 15, 360);
      text("斜率 " + slope, 15, 390);

      text("字体与条纹调整 " + typePush, 15, 510);
      text("相机缩放 " + zoomCamera, 15, height - 20);

      text("预设", 150,height-70);
      text("无条纹", 170,79);

      translate(0, height);
      rotateZ(-PI / 2);
      text("相机X旋转 " + xRotCamera, 45, 20);
      text("相机Y旋转 " + yRotCamera, 45, 60);
      text("相机Z旋转 " + zRotCamera, 45, 100);
      text("分段开关与颜色", (height-230),140);
      text("背景色", (height-315),140);
    }
  pop();
