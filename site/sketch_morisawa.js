...
function setup() {
  ...
  prideButton = createButton('彩虹!'); prideButton.position(15,15); prideButton.mousePressed(pride);
  ...
  moonSet = createButton('月亮'); moonSet.position(530,height-30); moonSet.mousePressed(moon);
  postSpaceSet = createButton('段后间距'); postSpaceSet.position(530,height-50); postSpaceSet.mousePressed(postSpace);
  crossSet = createButton('  X  '); crossSet.position(588,height-30); crossSet.mousePressed(cross);
  bridgeSet = createButton('桥接'); bridgeSet.position(615,height-50); bridgeSet.mousePressed(bridge);
  whitneySet = createButton('惠特尼'); whitneySet.position(620,height-30); whitneySet.mousePressed(whitney);
  beachSet = createButton('海滩'); beachSet.position(670,height-50); beachSet.mousePressed(beach);
  xFluxSet = createButton('后退'); xFluxSet.position(685,height-30); xFluxSet.mousePressed(xFlux);

  fluxLoopSave = createButton('波动循环'); fluxLoopSave.position(770,height-50); fluxLoopSave.mousePressed(fluxLoop);
  scrollSave = createButton('滚动'); scrollSave.position(770,height-30); scrollSave.mousePressed(scroll);
}
...
function draw() {
  ...
  textSize(9);
    if(fluxSave == true || scrollSave == true){
      ...
      text("行 " + rows, 10, height-55);
      text("粗细 " + typeStroke, 10, height-25);
      text("字距 " + tracking, 125, height-55);
      text("滚动速度 " + speed, 125, height-25);
      text("行距 " + lineSpace, 235, height-55);
      text("哑光 " + padding, 235, height-25);

      text("镜像", 375, height-44);
      text("翻转速度", 375, height-28);
      text("行波动", 375, height-12);

      text("字体", 480, height-43);
      text("背景", 480, height-13);

      text("预设", 530, height-57);
      text("保存", 770, height-57);
    }
