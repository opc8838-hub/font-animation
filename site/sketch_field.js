...
//  exportButton = createButton('保存循环'); exportButton.position(140,10); exportButton.mousePressed(saveLoop);
  ...
  presetStacks = createButton('堆叠'); presetStacks.position(140,height-30); presetStacks.mousePressed(stackSet);
  presetBricks = createButton('砖块'); presetBricks.position(200,height-30); presetBricks.mousePressed(brickSet);
  presetSimpleZ = createButton('简单Z'); presetSimpleZ.position(260,height-30); presetSimpleZ.mousePressed(simpleZSet);
  presetComplexZ = createButton('复杂Z'); presetComplexZ.position(335,height-30); presetComplexZ.mousePressed(complexZSet);
  presetZebra = createButton('斑马'); presetZebra.position(420,height-30); presetZebra.mousePressed(zebraSet);
  presetHarlequin = createButton('小丑格'); presetHarlequin.position(480,height-30); presetHarlequin.mousePressed(harlequinSet);
...
function draw(){
  ...
  zoomCamera = zoomCameraSlider.value();
    ...
    textSize(9);
    text("列数 " + column,25,16);
    text("行数 " + row,25,46);
    text("字距 " + tracking,25,76);
    text("行距 " + lineSpace,25,106);

    text("横向缩放 " + typeX,25,146);
    text("纵向缩放 " + typeY,25,176);
    text("字重 " + typeStroke,25,206);

    text("波浪速度 " + speed,25,246);
    text("横向尺寸" + xOffset,25,276);
    text("纵向尺寸 " + yOffset,25,306);

    text("Z轴振幅 " + zWave,25,346);
    text("偏移",150,360);
    text("X轴振幅 " + xWave,25,376);
    text("偏移",150,390);
    text("Y轴振幅 " + yWave,25,406);
    text("偏移",150,420);
    text("Z平滑 " + yWavezRot,45,435);
    text("横向拉伸 " + yWavexStr,110,435);
    text("横向拉伸 " + xStrechWave,25,466);
    text("偏移",150,480);
    text("纵向拉伸 " + yStrechWave,25,496);
    text("偏移",150,510);
