...
function preload(){
  font0 = loadFont('assets/NotoSansSC-Regular.ttf');

  font1 = loadFont('assets/NotoSansSC-Regular.ttf');
  font2 = loadFont('assets/NotoSansSC-Regular.ttf');
  font3 = loadFont('assets/NotoSansSC-Regular.ttf');
  font4 = loadFont('assets/NotoSansSC-Regular.ttf');
  font5 = loadFont('assets/NotoSansSC-Regular.ttf');
  font6 = loadFont('assets/NotoSansSC-Regular.ttf');
  font7 = loadFont('assets/NotoSansSC-Regular.ttf');
  font8 = loadFont('assets/NotoSansSC-Regular.ttf');
}
...
function setup() {
  ...
  stripRadio.option('text', '文字');
  stripRadio.option('gradientH', '横向渐变');
  stripRadio.option('gradientV', '纵向渐变');
  stripRadio.option('stripes', '条纹');
  stripRadio.option('all', '全部混合');
  stripRadio.option('mix', '动态混合');
  stripRadio.position(120,height-125); stripRadio.style('width','90px'); stripRadio.selected('text'); stripRadio.changed(setStripChoice);
  ...
  b4Buntton = createButton('投票'); b4Buntton.position(380,height-105); b4Buntton.mousePressed(b4);
  b2Buntton = createButton('梦游者'); b2Buntton.position(380,height-80); b2Buntton.mousePressed(b2);
  b3Buntton = createButton('多轨'); b3Buntton.position(380,height-55); b3Buntton.mousePressed(b3);
  b1Buntton = createButton('多汁'); b1Buntton.position(380,height-30); b1Buntton.mousePressed(b1);
  b5Buntton = createButton('是与'); b5Buntton.position(480,height-105); b5Buntton.mousePressed(b5);
  b6Buntton = createButton('内核'); b6Buntton.position(480,height-80); b6Buntton.mousePressed(b6);
  b7Buntton = createButton('鳗鱼与风'); b7Buntton.position(480,height-55); b7Buntton.mousePressed(b7);
  button = createButton('重置节点'); button.position(480,height-30); button.mousePressed(resetPoints);
...
function draw() {
  ...
  textSize(10);
  textFont(font0);
  text("条带高度 " + stripHeight,15,height-90);
  text("条带数量 " + stripCount,15,height-55);
  text("轮廓",32,height-32);
  text("圆角端点",32,height-12);
  text("文字",145,height-113);
  text("渐变色一",145,height-93);
  text("渐变色二",145,height-73);
  text("条纹",145,height-53);
  text("全部",145,height-34);
  text("混合",145,height-15);

  text("渐变节点",220,height-110);
  text("背景",220,height-18);
  text("前景",300,height-18);

  text("预设",380,height-110);

  textSize(12);
  text("点击拖动以放置节点，按回车开始新线条。",10,height-170);
...
function setStripChoice(){
  if(stripRadio.value() == 'text'){
    stripChoice = 0;
  } else if(stripRadio.value() == 'gradientH'){
    stripChoice = 1;
  }  else if(stripRadio.value() == 'gradientV'){
    stripChoice = 2;
  }  else if(stripRadio.value() == 'stripes'){
    stripChoice = 3;
  }  else if(stripRadio.value() == 'all'){
    stripChoice = 4;
  }  else if(stripRadio.value() == 'mix'){
    stripChoice = 5;
  }
}
