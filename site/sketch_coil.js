...
function setup() {
  ...
  //exportButton = createButton('保存循环'); exportButton.position(140,20); exportButton.mousePressed(saveLoop);
  prideButton = createButton('彩虹!'); prideButton.position(140,45); prideButton.mousePressed(pride);
    
//  resetSet = createButton('重置'); resetSet.position(25,height-60); resetSet.mousePressed(reset);
  wideSet = createButton('加宽'); wideSet.position(25,height-60); wideSet.mousePressed(wide);
  superSet = createButton('超级'); superSet.position(75,height-60); superSet.mousePressed(tall);
  amoebaSet = createButton('变形虫'); amoebaSet.position(130,height-60); amoebaSet.mousePressed(amoeba);
  spacerSet = createButton('间隔'); spacerSet.position(200,height-60); spacerSet.mousePressed(spacer);	
  kittySet = createButton('小猫'); kittySet.position(260,height-60); kittySet.mousePressed(kitty);	
  
  hourglassSet = createButton('沙漏'); hourglassSet.position(25,height-35); hourglassSet.mousePressed(hourglass);	
  starSet = createButton('星星'); starSet.position(100,height-35); starSet.mousePressed(star);	
  star2Set = createButton('锯齿星'); star2Set.position(145,height-35); star2Set.mousePressed(star2);	
  pretzelSet = createButton('蝴蝶结'); pretzelSet.position(200,height-35); pretzelSet.mousePressed(pretzel);	
  lemniscateSet = createButton('无穷环'); lemniscateSet.position(260,height-35); lemniscateSet.mousePressed(lemniscate);	
...
function draw() {
  ...
  noStroke();
  textSize(9);
  text("横向缩放 " + typeX, 25, 20);
  text("纵向缩放 " + typeY, 25, 50);
  text("字重 " + typeStroke, 25, 80);

  text("数量 " + ribbonCount, 25, 130);
  text("尺寸 " + ribbonSize, 25, 160);
  text("端帽", 25, 190);
  text("圆角", 55, 207);
  text("扁平", 55, 228);
  
  text("螺旋半径 " + radius, 25, 260);
  text("间距 " + tracker, 25, 290);
  text("起始 " + spiralStart, 25, 320);
  text("旋转 " + spin, 25, 350);

  text("波浪尺寸 " + waveSize, 25, 410);
  text("波浪数 " + waveCount, 25, 440);
  text("波浪速度 " + waveSpeed, 25, 470);
  text("倾斜 " + slope, 25, 500);  
