...
//  } else {
    ...
    if (c1 == 'A' || c1 == 'a') {
      ...
      drawFallbackGlyph(c1, 0, 0);
    }
//  }
}

function drawFallbackGlyph(glyph, stretchX, stretchY) {
  var signedGlyphWidth = typeX + (stretchX || 0);
  var glyphWidth = Math.max(1, Math.abs(signedGlyphWidth));
  var glyphHeight = Math.max(1, typeY + (stretchY || 0));
  var glyphColor = typeof strkColor !== 'undefined'
    ? strkColor
    : (typeof textColor !== 'undefined' ? textColor : 0);
  ...
  translate(signedGlyphWidth / 2, glyphHeight / 2);
  scale(signedGlyphWidth / naturalWidth, 1);
