...
//  noSmooth();
  ...
  basicSet = createButton('基础'); basicSet.position(25,height-60); basicSet.mousePressed(basic);
  streamerSet = createButton('飘带'); streamerSet.position(75,height-60); streamerSet.mousePressed(streamer);
  terraceSet = createButton('阶梯'); terraceSet.position(150,height-60); terraceSet.mousePressed(terrace);
  linkSet = createButton('连接'); linkSet.position(210,height-60); linkSet.mousePressed(link);
  seaSet = createButton('海面'); seaSet.position(253,height-60); seaSet.mousePressed(sea);
  riverSet = createButton('河流'); riverSet.position(295,height-60); riverSet.mousePressed(river);
  
  webRibbonSet = createButton('网页艺术'); webRibbonSet.position(25,height-35); webRibbonSet.mousePressed(webRibbon);
  primarySet = createButton('原色'); primarySet.position(90,height-35); primarySet.mousePressed(primary);
  snakeSet = createButton('蛇形'); snakeSet.position(153,height-35); snakeSet.mousePressed(snake);
  hotcoldSet = createButton('冷暖'); hotcoldSet.position(208,height-35); hotcoldSet.mousePressed(hotcold);
  trackSet = createButton('轨道'); trackSet.position(280,height-35); trackSet.mousePressed(track);
  track2Set = createButton('轨道二'); track2Set.position(330,height-35); track2Set.mousePressed(track2);
  
  saveLoopSet = createButton('保存循环'); saveLoopSet.position(150, 20); saveLoopSet.mousePressed(saveLoop); 
...
fill(textColor);
  if(gifRecord == false){
      push();
        ...
        text("分段间距 " + segmentSpace,25,30);
        text("分段数量 " + segmentCount,25,60);
        text("字体高度 " + typeHeight,25,90);
        text("字距 " + tracking,25,120);
        text("字体粗细 " + typeStroke,25,150);

        text("速度 " + speed,25,210);

        text("丝带高度 " + depth,25,270);
        text("丝带拉伸 " + middleStretchSlider.value(),25,300);
        text("丝带数量 " + count,25,330);
        text("丝带间距 " + zSpace,25,360);
        text("丝带偏移 " + xSpace,25,390);
        text("交替 ",70,420);
        text("缩放 " + scalerSlider.value(),25,440);

        text("X轴旋转 " + rotX,25,height-170);
        text("Y轴旋转 " + rotY,25,height-140);
        text("Z轴旋转 " + rotZ,25,height-110);

        text("预设", 25,height-70);
