...
function drawFallbackCornerGlyph(glyph) {
  var topCenter = p5.Vector.lerp(TL, TR, 0.5);
  var bottomCenter = p5.Vector.lerp(BL, BR, 0.5);
  var center = p5.Vector.lerp(topCenter, bottomCenter, 0.5);
  var glyphWidth = Math.max(1, p5.Vector.dist(TL, TR));
  var glyphHeight = Math.max(1, p5.Vector.dist(topCenter, bottomCenter));
  var angle = atan2(TR.y - TL.y, TR.x - TL.x);

  push();
  translate(center.x, center.y, center.z + 1);
  rotateZ(angle);
  noStroke();
  fill(strkColor);
  textFont(font);
  ...
  var naturalWidth = Math.max(1, textWidth(glyph));
  scale(glyphWidth / naturalWidth, 1);
  text(glyph, 0, -glyphHeight * 0.06);
  pop();
}
