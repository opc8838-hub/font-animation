(() => {
  ...
  function renderPairEditor() {
    ...
    phrasePairs.forEach((pair, index) => {
      ...
      row.querySelector(".pair-apply-text-color-all")?.addEventListener("click", () => {
        const leadValue = leadColor.value;
        const suffixValue = suffixColor.value;
        const punctuationValue = punctuationColor.value;
        phrasePairs.forEach((item) => {
          item.leadColor = leadValue;
          item.suffixColor = suffixValue;
          item.punctuationColor = punctuationValue;
        });
        inputs.textColor.value = leadValue;
        renderPairEditor();
        updateEditorState();
        const status = $("#schemeStatus");
        if (status) status.textContent = "文字颜色已应用到全部段落。";
      });
      row.querySelector(".pair-apply-background-color-all")?.addEventListener("click", () => {
        const value = backgroundColor.value;
        phrasePairs.forEach((item) => { item.backgroundColor = value; });
        inputs.backgroundColor.value = value;
        renderPairEditor();
        updateEditorState();
        const status = $("#schemeStatus");
        if (status) status.textContent = "背景颜色已应用到全部段落；图片、GIF 和视频保持独立。";
      });
  ...
  const phaseDefinitions = [
    ...
    { key: "hold", index: "02", label: "主词停顿", className: "is-orbit" },
    { key: "anticipation", index: "03", label: "居中预备", className: "is-hold" },
    { key: "reveal", index: "04", label: "后句接入", className: "is-contact" },
  ...
  $("#addPairButton").addEventListener("click", () => {
    ...
    const previous = phrasePairs[phrasePairs.length - 1];
    phrasePairs.push({ id: `continuation-row-${nextRowId++}`, lead: "New", suffix: "continuation.", revealStyle: "whole", fontFamily: previous?.fontFamily || "inherit", leadColor: normalizeColor(previous?.leadColor, inputs.textColor.value), suffixColor: normalizeColor(previous?.suffixColor, inputs.textColor.value), punctuationColor: normalizeColor(previous?.punctuationColor, inputs.textColor.value), leadFontSize: Number(previous?.leadFontSize || inputs.leadFontSize.value), suffixFontSize: Number(previous?.suffixFontSize || inputs.suffixFontSize.value), leadLetterSpacing: 0, suffixLetterSpacing: 0, pairGap: Number(previous?.pairGap || inputs.wordGap.value), sweepEnabled: false, sweepColors: [], sweepDuration: 100, leadIntroStyle: "original", ...rowMotionDefaults("whole", normalizeColor(previous?.backgroundColor, inputs.backgroundColor.value)) });
