...
function render() {
  ...
  grid.innerHTML = visible.map(([slug, zh, en, , categoryName]) => {
    ...
    const imageName = slug === "crashclock" ? "final_crashclock.png" : ["iconburst", "focuswheel", "gradienttype", "glyphrelay", "colorcanvas", "liquidtype", "scrapbin", "terminalbrand", "slotstories", "colorrecompose", "phrasebuild", "switchdrop"].includes(slug) ? `final_${slug}.svg` : `final_${slug}.png`;
