...
const featuredPreviews = new Set(["textswell", "impactbuild", "currentwall", "verticalwall", "iconburst", "glyphmorph", "sproutshift", "mistlift", "typecascade", "dotresolve", "continuation", "beforeafter", "shutterafter", "pathwriter", "scrapbin"]);
...
function render() {
  ...
  grid.innerHTML = visible.map(([slug, zh, en, , categoryName]) => {
    ...
    const preview = slug === "continuation"
      ...
      : ["sproutshift", "mistlift", "typecascade", "dotresolve"].includes(slug)
        ? `<video class="effect-loop" src="assets/previews/${slug}-card.mp4?v=20260902-2" autoplay muted loop playsinline preload="metadata" aria-label="${zh}动态效果预览"></video>`
      : slug === "currentwall"
