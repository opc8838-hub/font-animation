  ...
  grid.innerHTML = visible.map(([slug, zh, en, , categoryName]) => {
    ...
    const preview = slug === "split-flip"
      ...
      : ["sproutshift", "mistlift", "typecascade", "dotresolve", "glyphreveal"].includes(slug)
        ? `<video class="effect-loop" src="assets/previews/${slug}-wide-card.mp4?v=20260909-1" autoplay muted loop playsinline preload="metadata" aria-label="${zh}动态效果预览"></video>`
      : slug === "currentwall"
      ...
      : slug === "ribbonink"
        ? `<video class="effect-loop" src="assets/previews/ribbon-ink-card.mp4?v=20260909-1" autoplay muted loop playsinline preload="metadata" aria-label="${zh}动态效果预览"></video>`
      : slug === "scrapbin"
