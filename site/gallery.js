...
function render() {
  ...
  grid.innerHTML = visible.map(([slug, zh, en, , categoryName]) => {
    ...
    const preview = slug === "continuation"
      ...
      : slug === "ribbonink"
        ? `<video class="effect-loop" src="assets/previews/ribbon-ink-card.mp4?v=20260901-2" autoplay muted loop playsinline preload="metadata" aria-label="${zh}动态效果预览"></video>`
      : slug === "scrapbin"
