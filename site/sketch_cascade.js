...
//  pixelDensity(1);
  ...
  checkerSet = createButton('棋盘格'); checkerSet.position(25,height-210); checkerSet.mousePressed(checker);
  cascadeSet = createButton('瀑布'); cascadeSet.position(90,height-210); cascadeSet.mousePressed(cascade);
  classicSet = createButton('经典'); classicSet.position(25,height-190); classicSet.mousePressed(classic);
  mosaicSet = createButton('马赛克'); mosaicSet.position(80,height-190); mosaicSet.mousePressed(mosaic);
  gradientCheckerSet = createButton('跑马灯'); gradientCheckerSet.position(25,height-170); gradientCheckerSet.mousePressed(gradientChecker);
  runningSet = createButton('运行'); runningSet.position(80,height-170); runningSet.mousePressed(running);
  salmonSet = createButton('鲑鱼'); salmonSet.position(25,height-150); salmonSet.mousePressed(salmon);
  gridSet = createButton('网格'); gridSet.position(85,height-150); gridSet.mousePressed(grid);
  webartSet = createButton('网页艺术'); webartSet.position(25,height-130); webartSet.mousePressed(webart);
  sparkleSet = createButton('闪烁'); sparkleSet.position(90,height-130); sparkleSet.mousePressed(sparkle);
  pixelGradientSet = createButton('像素渐变'); pixelGradientSet.position(25,height-110); pixelGradientSet.mousePressed(pixelGradient);
...
function draw() {
  ...
  if(gifRecord==false){
      ...
      text("字重 " + typeStroke, 25, 50);
      text("字距 " + tracking, 25, 80);
      text("行距 " + lineSpaceSlider.value(), 25, 110);

      text("行数 " + rows, 25, 160);
      text("镜像", 43,191);

      text("波长 " + waveLength, 25, 220);
      text("波浪速度 " + waveSpeedSlider.value(), 25, 250);
      text("倾斜 " + slope, 25, 280);

      text("渐变模式", 172,90);
      text("无条纹", 172,70);
      text("背景色", 148,275);

      text("预设", 25, height-220);
