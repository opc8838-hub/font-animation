...
- Finale centering regression: the previous spacing plan released an icon's widened letter slot for 260 ms *after* its matched cut. The last icon was already invisible while the restored word remained 10px left of center at 1920×1080; the loop restarted before the slot returned to normal. The slot and icon now contract together during the 160 ms before the cut, leaving the restored word centered from its first fully visible frame. Entry, collision, color, replacement order and phase lengths remain frozen.
