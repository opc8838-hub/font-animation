...
- Finale rollback after rejected refinements: `d064cf8` was an incorrect comparison baseline because it already contained the newly introduced animated letter-slot layout. The user rejected that change and the subsequent shrink/recenter attempts. `site/iconburst.js` is restored exactly from `dc44f77`, before the responsive-finale work began. This removes the added slot reservation, letter reflow, layout fitting and extra settling interval; the original in-place substitutions, sizes, movement and timing are restored. The earlier hover-drift and canvas-zoom-scope fixes are included in that restored revision. Responsive finale sizing remains unresolved and must not be reintroduced by changing the original choreography.
