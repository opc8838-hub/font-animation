...
`leveling up` 从画面中心跳出并连续带动多行弹满，不插入静止停留；收束后以水流同款扫变完成末行字母与左右摆动图标的切换，最后快速消失。
...
| 0.92–1.44s | 用户修订 | 末行扫变 | 指定字母依次替换为图标，再依次恢复 | 沿用水流的“字母→图标→字母”扫变；图标围绕自身中心以正负角度快速摆动 | high |
...
- Scale behavior: 开场从中心快速跳出并在铺满阶段继续回弹；收尾图标动态撑开相邻文字并左右摆动
...
- Holds with residual motion: 满屏没有停留；图标出现期间持续左右摆动
...
- 源片末行最终停在全图标态；本次按用户明确要求改为水流同款双向扫变，并增加图标自身左右摆动。
