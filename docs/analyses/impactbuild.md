...
- 2026-09-01: the second approved exports replace the bundled default scheme and gallery-card MP4 without changing choreography; their cache versions are bumped so existing browsers load the new files immediately.
- 2026-09-01: the approved `impact-build-scheme.json` is now the bundled default loaded by preview, first visit, gallery entry, and Restore Default, with the embedded constants retained as an offline fallback. Entering from the gallery applies that approved state once and then removes the `from` flag so later edits survive reload. The approved 1920×1080/60fps export is the gallery card loop, replacing the responsive iframe so the card shows the exact signed-off composition.
- 2026-08-31: v17 completes the shared scheme and export workflow without changing choreography. Save writes the complete v7 state (including afterimage controls, word settings, icon tuning, background media, dimensions, and export settings) to local storage and JSON; import validates the JSON root before rebuilding dependent UI; restore and clear remove stale storage before applying their canonical states. PNG/GIF/MP4 now share consistent busy/error/success feedback, with size/FPS metadata on completed animated exports.
