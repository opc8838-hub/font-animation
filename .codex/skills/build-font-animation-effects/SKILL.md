...
1. For any editor UI or interaction work, read [references/cellmotion-editor-standard.md](references/cellmotion-editor-standard.md), [references/editor-interaction-contract.md](references/editor-interaction-contract.md), and [references/timeline-color-contract.md](references/timeline-color-contract.md). Type Cascade is the approved shell reference; matching only its colors or card shapes does not satisfy the standard.
...
- Give every effect the approved CellMotion choreography UI, not merely a functional timeline. Reuse `me-choreo-track`, `me-choreo-scroll`, `me-choreo-bar`, `me-choreo-block`, `me-choreo-playhead`, and `me-choreo-legend`; preserve rounded multi-color phase blocks, the visible playhead, legend/detail rows, and click-to-seek behavior while naming phases for that effect. Follow the global timeline color contract: enabled phases are chromatic, gray is not a phase fill, and fluorescent lime is only one palette member.
...
- Check that timeline blocks, durations, active beat, playhead, and click-to-seek all stay synchronized after timing edits. Also apply the timeline color contract: use the required number of non-gray semantic fills, keep phase-color mapping stable, and reject fluorescent-lime-plus-gray fallback timelines.
